...
function Player() {
  ...
  useEffect(() => {
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
      ...
      player.addListener('account_error', ({ message }: { message: string }) => {
        console.error('Erro de conta:', message, 'O usuário precisa de Spotify Premium?');
      });
  ...
  return (
    <article
      className={`${styles.playerContainer} ${showPlayer ? styles.showPlayer : styles.hidePlayer}`}
      onClick={(e) => {
        e.preventDefault();
    ...
    >
      {isConnected && (
        <BaseModal
          ...
          footer={null}
        >
          <div className={styles.connectModalWrapper} onClick={handleModalClose}>
            ...
            <div className={styles.connectModalChildren}>
              <span>3. Selecione o dispositivo chamado &quot;Reverbrate Player&quot;</span>
            </div>
          </div>
        </BaseModal>
      )}
