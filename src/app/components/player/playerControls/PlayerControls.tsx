...
function PlayerControls({ progress, dispatchFn, duration, player, isPaused }: PlayerControlsProps) {
  ...
  const handleProgressSeek = (newProgressPercent: number) => {
    ...
    if (player && typeof player.seek === 'function') {
      ...
      console.warn('Objeto do player do Spotify não disponível ou sem método seek.');
    }
  };
  ...
  return (
    <section className={styles.playerControlContainer}>
      <div className={styles.controlsWrapper}>
        <button className={styles.skipButton} onClick={() => player.previousTrack()}>
        ...
        <button className={styles.skipButton} onClick={() => player.nextTrack()}>
          <span className={styles.skipIcon}>
      ...
      <div className={styles.musicBarContainer}>
        <span className={styles.musicMinutes}>{formatMillisecondsToMMSS(displayProgressMs)} </span>
        <ProgressBar
          progress={duration > 0 ? (displayProgressMs / duration) * 100 : 0}
          onSeek={handleProgressSeek}
          onDragUpdate={handleProgressDragUpdate}
        />
        <span className={styles.musicMinutes}>{formatMillisecondsToMMSS(duration)}</span>
      </div>
    </section>
  );
}
